(function () {
    ...
    WMD = function (input, toolbar, options) {
        options = extend({preview: null, previewEvery: 0.5, showdown: null, lineLength: 40, commands: "strong em spacer a blockquote code img spacer ol ul h hr", commandTable: {}, lang: "EN"}, options);
        ...
        var obj = {}, shortcuts = {}, previewInterval, lastValue = "";
        ...
        function buildToolbar() {
            var ul, i, key, definition, builder, command, commands = options.commands.split(" ");
            if (toolbar) {
                toolbar.innerHTML = "";
        ...
        function createEvents() {
            ...
            addEvent(input, browser.Opera ? "keypress" : "keydown", function (event) {
                var ev = event || window.event, keyCode = ev.keyCode || ev.which, keyChar = String.fromCharCode(keyCode).toLowerCase();
            ...
            addEvent(input, "keyup", function (event) {
                var ev = event || window.event, keyCode = ev.keyCode || ev.which, meta = ev.shiftKey || ev.ctrlKey || ev.metaKey, state, chunk;
                if (!meta && keyCode === 13) {
            ...
            if (browser.IE) {
                addEvent(input, "keypress", function (event) {
                    var ev = event || window.event, keyCode = ev.keyCode || ev.which;
                    if (keyCode === 27) {
        ...
        buildToolbar();
        createEvents();
        return extend(obj, {input: input, options: options, ieClicked: false, ieRange: null});
    };
    ...
    function extendRegExp(regex, pre, post) {
        var pattern = regex.toString(), flags = "", result;
    ...
    function randomString(length, options) {
        options = extend({numbers: false, lower: true, upper: true, other: false}, options);
    ...
    function removeEvent(element, event, callback, cache) {
        ...
        for (; i < cache.length; i++) {
            if (cache[i].element === element && cache[i].event === event && cache[i].callback === callback) {
                cached = cache[i];
                break;
            }
        }
    ...
    Translator_EN = {urlFieldInsertion: '<span class="note">To add a tool-tip, place it in quotes after the URL (e.g., <strong>http://google.com "Google"</strong>)</span>', linkText: "link text", imageAlt: "image alt", listItem: "List item", strongTitle: "加粗 <strong> Ctl+B", emTitle: "斜体 <em> Ctl+I", aTitle: "超链接 <a> Ctl+L", blockquoteTitle: "引用 <blockquote> Ctl+Q", codeTitle: "代码 <pre><code> Ctl+K", imgTitle: "图片 <img>", olTitle: "有序列表 <ol> Ctl+O", ulTitle: "无序列表 <ul> Ctl+U", hTitle: "标题 <h1>/<h2> Ctl+H", hrTitle: "分割线 <hr> Ctl+R", strongText: "<i class='icon-bold'></i>", emText: "<i class='icon-italic'></i>", aText: "<a data-toggle=\"modal\" data-target=\".bs-addlink-modal-sm\"><i class='icon-link'></i></a>", blockquoteText: "<i class='icon-quote-left'></i>", codeText: "<i class='icon-laptop'></i>", imgText: "<a data-toggle=\"modal\" data-target=\".bs-upload-modal-sm\"><i class='icon-picture'></i></a>", olText: "<i class='icon-list-ol'></i>", ulText: "<i class='icon-list-ul'></i>", hText: "<i class='icon-th-list'></i>", hrText: "<i class='icon-cut'></i>", insertImage: "Insert image", imageURL: "Image URL", enterCodeHere: "enter code here", blockquote: "Blockquote", insertLink: "Insert link", linkURL: "Link URL", submit: "Submit", cancel: "Cancel"};
    Translator_RU = {urlFieldInsertion: '<span class="note">(например, <strong>http://google.com</strong>)</span>', linkText: "название ссылки", imageAlt: "описание картинки", listItem: "текст", strongTitle: "Жирный <strong> Ctl+B", emTitle: "Курсив <em> Ctl+I", aTitle: "Вставить ссылку <a> Ctl+L", blockquoteTitle: "Ыитировать <blockquote> Ctl+Q", codeTitle: "Вставить пример кода <pre><code> Ctl+K", imgTitle: "Вставить картинку <img> Ctl+G", olTitle: "Нумерованный список <ol> Ctl+O", ulTitle: "Маркированный список <ul> Ctl+U", hTitle: "Вставить заголовок <h1>/<h2> Ctl+H", hrTitle: "Горизонтальная черта <hr> Ctl+R", strongText: "Bold", emText: "Italic", aText: "Link", blockquoteText: "Blockquote", codeText: "Code", imgText: "Image", olText: "Numbered List", ulText: "Bulleted List", hText: "Heading", hrText: "Horizontal Rule", insertImage: "Вставить картинку", imageURL: "Адрес картинки", enterCodeHere: "вставьте код сдесь", blockquote: "текст цитаты", insertLink: "Вставить ссылку", linkURL: "Ссылка", submit: "Ок", cancel: "Отмена"};
    ...
    InputState = function (wmd) {
        var obj = {}, input = wmd.input;
        obj = extend(obj, {scrollTop: 0, text: "", start: 0, end: 0, getChunk: function () {
            ...
            var range, fixedRange, markedRange, rangeText, len, marker = "\x07";
            if (visible(input)) {
                if (input.selectionStart || input.selectionStart === 0) {
    ...
    Command = function (wmd, definition, runner, options) {
        ...
        var element, obj = {}, downCss = definition.css + options.downCssSuffix;
        ...
        return extend(obj, {draw: function (parent) {
            var span, downCss = definition.css + options.downCssSuffix;
            if (!element) {
            ...
            var state = new InputState(wmd), chunk = state.getChunk();
    ...
    extend(Command, {css: {base: "wmd-command", over: "wmd-command-over", down: "wmd-command-down"}, autoIndent: function (wmd, chunk, callback, args) {
        ...
        var cancel = document.createElement("a"), form = new Form(title, {dialog: true, onSubmit: onSubmit, onDestroy: onDestroy}), submitField = new Field("", "submit", {value: Translator.submit});
        ...
        var previousItemsRegex = /(\n|^)(([ ]{0,3}([*+-]|\d+[.])[ \t]+.*)(\n.+|\n{2,}([*+-].*|\d+[.])[ \t]+.*|\n{2,}[ \t]+\S.*)*)\n*$/, nextItemsRegex = /^\n*(([ ]{0,3}([*+-]|\d+[.])[ \t]+.*)(\n.+|\n{2,}([*+-].*|\d+[.])[ \t]+.*|\n{2,}[ \t]+\S.*)*)\n*/, finished = false, bullet = "-", num = 1, hasDigits, nLinesBefore, prefix, nLinesAfter, spaces;
        ...
        callback();
    }, runners: {a: function (wmd, chunk, callback, args) {
        ...
        var textBefore = /\S[ ]*$/.test(chunk.before), textAfter = /^[ ]*\S/.test(chunk.after), linesBefore = 1, linesAfter = 1;
        ...
        Command.runStrongEm(wmd, chunk, callback, extend({stars: 1, text: "emphasized text"}, args));
    }, h: function (wmd, chunk, callback, args) {
        args = args || {};
        callback = typeof callback === "function" ? callback : function () {
        };
        var headerLevel = 0, headerLevelToCreate, headerChar, len;
        ...
        Command.runStrongEm(wmd, chunk, callback, extend({stars: 2, text: "strong text"}, args));
    }, ul: function (wmd, chunk, callback, args) {
        Command.runList(wmd, chunk, callback, extend({tag: "ul"}, args));
    }}});
    Command.builtIn = {strong: {text: Translator.strongText, title: Translator.strongTitle, css: "wmd-strong", shortcut: "b"}, em: {text: Translator.emText, title: Translator.emTitle, css: "wmd-em", shortcut: "i"}, a: {text: Translator.aText, title: Translator.aTitle, css: "wmd-a", shortcut: "l"}, blockquote: {text: Translator.blockquoteText, title: Translator.blockquoteTitle, css: "wmd-blockquote", shortcut: "q"}, code: {text: Translator.codeText, title: Translator.codeTitle, css: "wmd-code", shortcut: "k"}, img: {text: Translator.imgText, title: Translator.imgTitle, css: "wmd-img", shortcut: "g"}, ol: {text: Translator.olText, title: Translator.olTitle, css: "wmd-ol", shortcut: "o"}, ul: {text: Translator.ulText, title: Translator.ulTitle, css: "wmd-ul", shortcut: "u"}, h: {text: Translator.hText, title: Translator.hTitle, css: "wmd-h", shortcut: "h"}, hr: {text: Translator.hrText, title: Translator.hrTitle, css: "wmd-hr", shortcut: "r"}, spacer: {css: "wmd-spacer", builder: Command.createSpacer}};
    Dialog = function (options) {
        var obj, element, overlay, events = [], options = extend({zIndex: 10, css: "wmd-dialog", overlayColor: "#FFFFFF", modal: true, closeOnEsc: true, insertion: null, onDestroy: null}, options);

        function build() {
            if (!element) {
                if (options.modal) {
                    overlay = new Overlay({color: options.overlayColor, zIndex: options.zIndex - 1});
                }
        ...
        obj = extend(obj, {destroy: function () {
    ...
    Overlay = function (options) {
        var obj = {}, events = [], element, iframe, options = extend({color: "#FFFFFF", zIndex: 9, scroll: true, opacity: 0.3}, options);

        function update() {
            var scroll, size;
            if (element) {
                scroll = {left: window.pageXOffset || document.documentElement.scrollLeft || document.body.scrollLeft, top: window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop};
    ...
    Form = function (title, options) {
        title = title || "";
        options = extend({css: "wmd-form", legendCss: "wmd-legend", errorCss: "wmd-error", requiredReason: "Required", dialogCss: "wmd-dialog", dialog: false, modal: true, dialogZIndex: 10, closeOnEsc: true, id: "", onSubmit: null, onDestroy: null}, options);
        var element, events = [], fields = [], fieldset, error, dialog;
        if (!options.id) {
        ...
        if (options.dialog) {
            dialog = new Dialog({modal: options.modal, zIndex: options.dialogZIndex, css: options.dialogCss, closeOnEsc: false, insertion: element});
        }
        addEvent(document, "keypress", function (event) {
        ...
        extend(element, {addField: function (key, field) {
            ...
            var hash = {}, missing = 0, field, value, type, i, n;
            for (i = 0, n = fields.length; i < n; i++) {
                field = fields[i].value;
    ...
    Field = function (label, type, options) {
        ...
        options = extend({required: false, inline: false, "float": false, items: null, itemsAlign: "left", css: "wmd-field", inputCss: "wmd-fieldinput", buttonCss: "wmd-fieldbutton", passwordCss: "wmd-fieldpassword", labelCss: "wmd-fieldlabel", inlineCss: "wmd-fieldinline", floatCss: "wmd-fieldfloat", errorCss: "wmd-fielderror", reasonCss: "wmd-fieldreason", hiddenCss: "wmd-hidden", value: "", group: "", id: "", insertion: null}, options);
        var element, labelElement, inner, inputs, errorElement, events = [], setFor = false;
        ...
        extend(element, {addEvent: function (event, callback) {
            var c = function () {
                callback(element);
            }, input, i, n;
            if (inputs) {
    ...
    extend(Field, {TYPES: ["button", "checkbox", "empty", "file", "hidden", "image", "password", "radio", "reset", "submit", "text", "select", "textarea"], createInput: function (parent, type, options) {
        var id = Field.getInputId(options), css = type === "button" || type === "submit" || type === "reset" ? options.buttonCss : options.inputCss, input = document.createElement("input");
        input.id = id;
        input.name = id;
        input.className = css;
        input.type = type;
        ...
        var i, n, id, span, label, name, input, inputs = [];
        ...
        var i, n, id = Field.getInputId(options), select, index;
        ...
        var id = Field.getInputId(options), input = document.createElement("textarea");
        input.id = id;
        input.name = id;
    ...
    LinkHelper = {add: function (chunk, linkDef) {
        var refNumber = 0, defsToAdd = {}, defs = "", regex = /(\[(?:\[[^\]]*\]|[^\[\]])*\][ ]?(?:\n[ ]*)?\[)(\d+)(\])/g;
        ...
        var form, urlField, submitted = false;
        callback = typeof callback === "function" ? callback : function () {
        };
        ...
        urlField = new Field(fieldLabel, "text", {required: true, value: "http://", insertion: Translator.urlFieldInsertion});
